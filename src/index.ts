...
export class MerkleDistributor {
  ...
  static async getDistributorStatus(con: Connection, merkleDistributorProgramId: string, claimProofEndpoint: string, mint: web3.PublicKey, base: web3.PublicKey) {
    try {
      ...
      const allDistributor: Record<string, DistributorResponse> = await res.json();
  ...
  async getUser(mint: web3.PublicKey, base: web3.PublicKey, claimant: web3.PublicKey): Promise<UserResponse | null> {
    try {
      const res = await fetch(`${this.claimProofEndpoint}/user/${mint.toBase58()}/${base.toBase58()}/${claimant.toBase58()}`);
  ...
  async claimToken(mint: web3.PublicKey, base: web3.PublicKey, claimant: web3.PublicKey) {
    const {
      provider: { connection },
      mdProgram,
    } = this;
    ...
    const user = await this.getUser(mint, base, claimant);
  ...
  async getClaimStatus(mint: web3.PublicKey, base: web3.PublicKey, claimant: web3.PublicKey): Promise<{ amount: BN; isClaimed: boolean } | null> {
